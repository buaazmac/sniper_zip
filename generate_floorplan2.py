...
area = {'core': {'exe': (5.71e-3, 1.43e-3),
				 ...
				 'ru': (0.61e-3, 0.61e-3)},
		'L3': (11.42e-3, 4.58e-3),
		'dram_cache': (11.42e-3, 16.00e-3)}

flr_files = ['core_layer_cache.flr', 'dram_layer_0_cache.flr', 'dram_layer_1_cache.flr', 'dram_layer_2_cache.flr', 'dram_layer_3_cache.flr', 'dram_layer_4_cache.flr']
...
def draw_dram(flrfile, x, y, layer):
	with file(flrfile, 'a') as f:
		...
		xx, yy = x, y
		width = float(area['dram_cache'][0]) / float(4)
		length = float(area['dram_cache'][1]) / float(8)
		for i in xrange(32):
			unitname = 'dram_cache_' + str(i) + '_' + str(layer)
			line = '%s\t%f\t%f\t%f\t%f\n' \
					% (unitname, width, float(2e-3), float(xx), float(yy))
			f.write(line)
			if i % 4 == 3:
				xx = x
				yy = yy + length
