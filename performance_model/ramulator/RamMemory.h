  ...
  long max_address;
          ...
          addr_bits(int(Level::MAX))
    {
        ...
        if (type != Type::RoBaRaCoCh && spec->standard_name.substr(0, 5) == "LPDDR")
            assert((sz[int(Level::Row)] & (sz[int(Level::Row)] - 1)) == 0);
        ...
        addr_bits[int(Level::MAX) - 1] -= calc_log2(spec->prefetch_size);
    ...
    void finish(void) {
      dram_capacity = max_address;
      int *sz = spec->org_entry.count;
      maximum_bandwidth = spec->speed_entry.rate * 1e6 * spec->channel_width * sz[int(Level::Channel)] / 8;
