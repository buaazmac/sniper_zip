...
def compute_dram_power(nread, nwrite, t, config):
  ...
  rd_per = 50
  if (nread + nwrite > 0):
    rd_per = round((100 * nread) / (nread + nwrite))
  tot_dyn = float(pt.power_table[rd_per][1]) + float(pt.power_table[rd_per][6])
  tot_stat = float(pt.power_table[rd_per][12])
  tot_power = float(pt.power_table[rd_per][13])
  print '-----[power] %f %f %f' % (tot_dyn, tot_stat, tot_power)
  return (tot_dyn / 1000.0, tot_stat / 1000.0, tot_power / 1000.0)
'''
...
# print 'power_results_line_=: ', line
...
# print 'power_results_line_no=: ', line
  ...
  print total_time
  dram_results_file = file('DramCacheAccess.txt')
  drams = dram_results_file.read().split('\n')[1:]
  dram_num, dram_stat_power = 0, 0
  for dram_line in drams:
    dram_data = dram_line.split(' ')
    if len(dram_data) < 2:
      continue
    dram_n = dram_data[0]
    dram_reads = int(dram_data[1])
    dram_writes = int(dram_data[2])
    dram_power = compute_dram_power(dram_reads, dram_writes, total_time[0], results['config'])	
    output2[dram_n] = dram_power[2]
    dram_stat_power = dram_stat_power + dram_power[1]
    dram_num = dram_num + 1
  output2['dram_ctr'] = float(dram_stat_power)/float(dram_num)
...
# keylist.sort()
  ...
  keylist.append('dram_ctr')
  for j in xrange(4):
    for i in xrange(32):
      keylist.append('dram_cache_%d_%d' % (i, j))
#keylist.append('null_%d' % j)
#output2['null_%d' % j] = 0
...
#keylist.append(key)
  ...
  file('hotspot_cache.input', "w").write(keystr + '\n' + valuestr)
