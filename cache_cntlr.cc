   ...
   m_shmem_perf_model(shmem_perf_model)
{
	...
	//is_last_level = is_last_level_cache;

	std::cout << "ahahaha: " << name << std::endl;
//STACKED_LLC Controller Initialization
#ifdef STACKED_LLC
	...
	if (is_last_level_cache && m_stacked_dram_cache == NULL) {
		...
		//m_stacked_dram_cache = new StackedDramCacheCntlr(st_set_n, StackedAssoc, StackedBlockSize, StackedPageSize);
	} else {
		//m_stacked_dram_cache = NULL;
	}
...
CacheCntlr::~CacheCntlr()
{
   ...
   #endif
   std::cout << "=-=deleting cache controller" << std::endl;
   if (m_stacked_dram_cache != NULL) {
	   std::cout << "...=0=deleting dram cache" << std::endl;
	   delete m_stacked_dram_cache;
	   m_stacked_dram_cache = NULL;
   }
  // delete aaa;
}
